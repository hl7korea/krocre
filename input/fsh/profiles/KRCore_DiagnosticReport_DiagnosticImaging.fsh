...
* ^purpose = "**KR CDI V2 진단영상검사** 클래스에서 정의한 진단 관련 항목들과 주요 추가 정보를 구조화하기 위해 정의한 Profile."
* . ^short = "KR Core DiagnosticReport Profile for Diagnostic Imaging"
* status MS
* status ^short = "진단영상검사 보고서 상태: registered | partial | preliminary | final +"
* status ^alias = "KRCDI: 진단영상검사 상태"
...
* code MS
* code from KR_Core_Diagnostic_Imaging_Codes (extensible)
* code ^short = "진단영상검사 보고서 항목 정보"
* code ^alias = "KRCDI: 진단영상검사명"
* code.coding.system 1.. MS
* code.coding.code 1.. MS
* subject only Reference(KRCore_Patient)
* subject MS
* subject ^short = "진단영상검사 대상 환자"
* issued ^short = "진단영상검사 결과 보고서 발행 일시"
* performer only Reference(KRCore_Practitioner or KRCore_PractitionerRole or KRCore_Organization or CareTeam)
* performer ^short = "진단영상검사 결과 보고 책임자"
* imagingStudy only Reference(KRCore_ImagingStudy)
* imagingStudy ^short = "진단영상 정보 참조"
* imagingStudy ^alias = "KRCDI: 진단영상"
* conclusion MS
* conclusion ^short = "진단영상검사 결과 판독 소견"
* conclusion ^alias = "KRCDI: 진단영상검사 결과"